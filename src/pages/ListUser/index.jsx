...
import { fetchGetProfiles, selectProfiles } from "../../store/profilesSlice";
...
function ListUsers() {
  ...
  const totalPages = Math.ceil(stateProfiles.data?.length / profilesPerPage);
  ...
  const profilesToDisplay = {
    data: stateProfiles.data?.slice(startIndex, endIndex),
  };

  return (
    <>
      {stateProfiles?.status === "loading" && (
        ...
        <div className="px-[2rem] pt-[2rem] min-h-screen w-screen flex flex-col justify-between">
          <table className="h-auto w-full border-collapse md:table">
            ...
            <tbody className="block md:table-row-group">
              {profilesToDisplay.data?.map((profiles) => (
                <tr className="bg-gray-300 border border-grey-500 md:border-none block md:table-row" key={profiles.id}>
